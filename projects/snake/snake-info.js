function showInfo() {
    ...
    An empty array named snake is declared, and the first index of that array is then defined as an object with the X and Y co-ordinates set to the center of the canvas using multiples of the 
    box variable. A food object is declared, using RNG to create random X and Y co-ordinates at which the food will appear throughout the game. Input is handled using a 'for' loop and event listeners set to the keycodes of the cursor
    keys, with a d (direction) variable used to ensure that the snake cannot reverse direction horizontally or vertically. A collision function will iterate through the snake array to check that the snake head X and Y co-ordinates are never 
    ...
    snake X or Y co-ordinates exceed the bounds of the canvas, or if the collosion function notes a collision, the game is over.   
