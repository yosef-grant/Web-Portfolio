function showInfo() {
    ...
    uses the running question variable to display the appropriate question, answer choices and image for the current question. Progress through the quiz is marked through a function which
    runs a for loop, which is set to terminate once 'qIndex' reaches the last question. The 'prog' div from the markup is assigned an ID, which increments in accordance with the loop. The countdown
    ...
    A percentage result is given based on the score divided by the number of questions in the quiz. A different message will display reflecting the player's percentage of correct answers.
